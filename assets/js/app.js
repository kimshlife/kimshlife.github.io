...
function hobbyOverview(){
  ...
  /* 고르는 이유가 바뀌어 온 흐름 — 카드 모양을 통계 카드와 맞춥니다 */
  const flow=[
    ['유년','호기심','집에 있던 PC에서 우연히 게임을 발견했습니다.'],
    ['초등','성취감','키우는 캐릭터가 자라는 것을 보며 오래 붙잡았고, 간단한 게임을 만들어 보기도 했습니다.'],
    ['중등','사람','사람의 심리에 관심이 생기면서 몰입형 스토리를 찾기 시작했습니다.'],
    ['고등','서브컬처','리듬게임으로 서브컬처를 접했고, 창작자의 처우를 보며 콘텐츠 산업을 생각하게 됐습니다.'],
    ['대학','오래가는 것','사업의 생존을 공부하면서, 라이브 서비스 게임이 어떻게 버티는지를 보게 됐습니다.']
  ];
  ...
  <div class="sub" style="margin-top:34px">${ico('idea',16)}게임을 고르는 이유가 바뀌어 온 흐름</div>
  <div class="flow">${flow.map(([s,tag,t],i)=>`<div class="flowstep">
    <span class="eyebrow">${String(i+1).padStart(2,'0')} · ${s}</span>
    <b>#${tag}</b><p>${t}</p></div>`).join('')}</div>
  <p class="cap" style="margin-top:20px">길드 운영 사례와 게임 분석 문서는 <a href="#/hobby/game/docs">문서</a>에 정리해 두었습니다.</p>`;
}
